...
$queryJobPermission = "SELECT u.netid, j.m FROM blast_jobs j, users u WHERE j.id=".$_GET['job']." AND j.userid=u.id AND u.netid=\"".$_SESSION['username']."\"";
$jobPermission = $sqlDataBase->query($queryJobPermission);
if($jobPermission)
{
	...
	$blastQueries = $sqlDataBase->query($queryBlastQueries);
        ...
        if(!empty($blastQueries))
        {
			 ...
			 if($_GET['show']!="running")
			 {
				if($jobPermission[0]['m']==0)
				{
					echo "| <a href=\"index.php?view=draw&queryid=".$assoc['id']."&job=".$_GET['job']."&chunksize=".$assoc['chunksize']."\">Graph Results</a>";
				}
