...
$configFilePath = $ENV{'WEBBLASTCONF'};
...
$queryPath      = $cfg->val('slave_paths','input_chunks_path');
$resultsPath    = $cfg->val('slave_paths','output_chunks_path');
...
sub SetNodeRunning {
	...
	$queryInsertStatusRunning ="INSERT INTO nodes (status,processes,hostname,pid,commandid)VALUES(2,0,\'$hostname\',$$,2)";
	$sthp = $dbhp->prepare($queryInsertStatusRunning);
	$sthp->execute();
	$nodeNumber=$dbhp->last_insert_id(undef, undef, undef, undef);
