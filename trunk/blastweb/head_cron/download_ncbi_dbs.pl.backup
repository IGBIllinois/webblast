...
#Number of nodes
#numNodes starts at 0 so 1 node is 0
####################################
my $numNodes = 23;
my $commonNodeName = "compute-0-";

#File with list of nodest to update
###################################
my $nodesListPath = "/var/www/html/webblast/nodes.txt";
...
my $sqlUser="blastWeb";
my $sqlPass="igb123";
my $sqlDatabase = "blastWeb";
my $sqlHost = "biocluster.igb.illinois.edu";
...
#paths on head node
my $extractDestination="/home/blastweb/blastdbs/ncbidbs";
my $compressedDownloads = "/home/blastweb/blastdbs/ncbi_compressed";
my $userDbsSource = "/home/blastweb/blastdbs/userdbs";
		...
		foreach $filePart (@{ $ncbiDbs{$shortNameDb} })
		{
			...
			{	
				...
				if($fileStatus==1)
				{
					if($shortNameDb)
					{
						if(open(NODES_LIST,$nodesListFile))
					    	{
							while(my $nodeName=<NODES_LIST>)
       	         					{
								print "\nrsync -au ".$extractDestination."/".$shortNameDb.$filePart."* ".$nodeName.":".$slaveRsyncdbs;
	                        				if(system("rsync -au ".$extractDestination."/".$shortNameDb.$filePart."* ".$nodeName.":".$slaveRsyncdbs) != 0)
	                        				{
	                                				$fileStatus=0;
	                                				$errorDetails=$errorDetails."\nFailed to rsync ".$shortNameDb.$filePart."* ncbi archive ".$nodeName;
	                               			 		print "\nFailed to rsync ncbi archive ".$nodeName;
       	                 					}
       	         					}
							close(NODES_LIST);
						}
						...
						{
							$fileStatus=0;
							$errorDetails=$errorDetails."\nFailed to open nodes list file ".$nodesListFile;
							print "\nFailed to open nodes list file".$nodesListFile;
						}
					}
				}
		...
		if($fileStatus == 1)
		{
			if($shortNameDb)
				...
				if(open(NODES_LIST,$nodesListFile))  	
					...
					while(my $nodeName=<NODES_LIST>)
					{
                                		print "\nrsync -au ".$extractDestination."/".$shortNameDb."*.*al ".$nodeName.":".$slaveRsyncdbs;
                                		if(system("rsync -au ".$extractDestination."/".$shortNameDb.".*al ".$nodeName.":".$slaveRsyncdbs) != 0)
                                		{
							if((-e $extractDestination."/".$shortNameDb.".nal")||(-e $extractDestination."/".$shortNameDb.".pal"))
							{ 
                                		        	$fileStatus=0;
								$errorDetails=$errorDetails."\nFailed to rsync ".$shortNameDb.".*al  ncbi archive ".$nodeName;
                                        	        	print "\nFailed to rsync file types to ncbi archive ".$nodeName;
							}
                                		}
					}
					close(NODES_LIST);
				...
				else
				{
					$fileStatus=0;
                                        $errorDetails=$errorDetails."\nFailed to open nodes list file ".$nodesListFile;
                                        print "\nFaile to open nodes list file".$nodesListFile
				}
			}
		}
