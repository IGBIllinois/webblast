...
my @nodMD5Exceptions = ("vector.tar.gz");
	...
	if($fileStatus == 1)
	{
		while(RunningJobsCount() > 0)
                ...
                print "\ncluster-fork mv -f ".$slaveBlastdbs."* ".$slaveBackupdbs;
                system("cluster-fork mv -f ".$slaveBlastdbs."* ".$slaveBackupdbs);

                print "\ncluster-fork mv -f ".$slaveRsyncdbs."* ".$slaveBlastdbs;
                system("cluster-fork mv -f ".$slaveRsyncdbs."* ".$slaveBlastdbs);
	...
	if($fileStatus==0)
	{
		...
		UpdateStatus($statusCompleted);	
        	...
        	sendEmail("nevoband\@igb.uiuc.edu","clcluster\@igb.uiuc.edu","Success Updating Blast DBs","Tries: ".$try." Success Code: ".$fileStatus);
	}
