...
function StatusTable($title,$type, $numCols, SQLDataBase $sqlDataBase)
{
	...
	if(isset($startNodes))
	{
		foreach($startNodes as $id=>$nodeInfo)
		{
			...
			$htmlString .="<td><img src=\"includes/graph.php?g=".$type."&m=load_one&z=small&c=Classroom%20Cluster&h=".$nodeInfo['hostname'].".local&l=e2ecff&v=0.00&x=0&n=0&r=hour&st=".Time()."\"></td>";
			$i++;
		}
	}
